...
export class ReaderComponent implements OnInit, OnDestroy {
  ...
  private avatarsRequestSub?: Subscription;
  ...
  ngOnInit() {
    ...
    this.saveProgressSubject.pipe(debounceTime(3000), takeUntil(this.destroy$)).subscribe(p => this.syncProgressToBackend(p));
    ...
    this.audioService.chapterEnd$.pipe(takeUntil(this.destroy$)).subscribe(() => {
      ...
      if (this.currentPage < this.totalPages) {
        setTimeout(() => {
          this.currentPage++;
          this.renderCurrentChapter(() => setTimeout(() => this.playAudio(), 600));
  ...
  loadInitialData() {
    ...
    this.api.get<any>(`library/inventory/${this.inventoryId}/`).pipe(takeUntil(this.destroy$)).subscribe({
      next: (inventory) => {
  ...
  loadChapters() {
    ...
    const params = new HttpParams().set('include_content', 'false');
    this.api.get(`library/inventory/${this.inventoryId}/chapters/`, params).pipe(takeUntil(this.destroy$)).subscribe({
      next: (res: any) => {
  ...
  renderCurrentChapter(afterRender?: () => void) {
    ...
    setTimeout(() => {
      ...
      this.parseAndRenderChapter(afterRender);
    }, 0);
  }
  ...
  parseAndRenderChapter(afterRender?: () => void) {
    ...
    if (typeof chapter.content_html !== 'string') {
      this.currentChapterPlainText = '';
      this.parsedBlocks = [];
      this.renderedBlocks = [];
      this.titleTokens = [];
      this.safeChapterHtml = this.sanitizer.bypassSecurityTrustHtml('');
      this.loadChapterContent(this.currentPage, afterRender);
      return;
    }
  ...
  private loadChapterContent(page: number, afterRender?: () => void) {
    ...
    const cacheKey = this.getChapterCacheKey(chapter, page);
    const cachedChapter = this.chapterContentCache.get(cacheKey);
    if (cachedChapter) {
      this.chapters[page - 1] = { ...chapter, ...cachedChapter };
      if (page === this.currentPage) {
        this.parseAndRenderChapter(() => {
          afterRender?.();
          this.runPendingChapterCallbacks(cacheKey);
        });
      }
      return;
    }

    if (this.chapterContentLoading.has(cacheKey)) {
      this.queueChapterCallback(cacheKey, afterRender);
      return;
    }

    this.queueChapterCallback(cacheKey, afterRender);
    this.chapterContentLoading.add(cacheKey);
    const requestToken = ++this.chapterRequestToken;
    const params = this.buildChapterRequestParams(chapter, page);

    this.api.get(`library/inventory/${this.inventoryId}/chapters/`, params).pipe(takeUntil(this.destroy$)).subscribe({
      next: (res: any) => {
        const loadedChapter = res?.chapter;
        if (!loadedChapter) {
          console.error('Respuesta de capítulo inválida.', res);
          return;
        }

        this.chapterContentCache.set(cacheKey, loadedChapter);
        this.chapters[page - 1] = { ...chapter, ...loadedChapter };

        if (page === this.currentPage && requestToken === this.chapterRequestToken) {
          this.parseAndRenderChapter(() => this.runPendingChapterCallbacks(cacheKey));
        }
      },
      error: (err) => {
        ...
        if (page === this.currentPage) {
          this.chapterTitle = 'Error';
          this.safeChapterHtml = this.sanitizer.bypassSecurityTrustHtml('<p>Hubo un error cargando este capítulo.</p>');
          this.isOverlayActive = false;
        }
      },
      complete: () => {
        this.chapterContentLoading.delete(cacheKey);
      }
    });
  }

  private preloadChapterContent(page: number) {
    if (page < 1 || page > this.totalPages) return;
    const chapter = this.chapters[page - 1];
    if (!chapter || typeof chapter.content_html === 'string') return;

    const cacheKey = this.getChapterCacheKey(chapter, page);
    if (this.chapterContentCache.has(cacheKey) || this.chapterContentLoading.has(cacheKey)) return;
  ...
  private buildChapterRequestParams(chapter: any, page: number): HttpParams {
    let params = new HttpParams();
    if (chapter?.id) {
      params = params.set('chapter_id', String(chapter.id));
    } else {
      params = params.set('order', String(chapter?.order ?? page));
    }
    return params;
  }

  private getChapterCacheKey(chapter: any, page: number): string {
    return String(chapter?.id || chapter?.order || page);
  }

  private queueChapterCallback(cacheKey: string, callback?: () => void) {
    if (!callback) return;
    const callbacks = this.pendingChapterCallbacks.get(cacheKey) || [];
    callbacks.push(callback);
    this.pendingChapterCallbacks.set(cacheKey, callbacks);
  }

  private runPendingChapterCallbacks(cacheKey: string) {
    const callbacks = this.pendingChapterCallbacks.get(cacheKey) || [];
    this.pendingChapterCallbacks.delete(cacheKey);
  ...
  playAudio() {
    ...
    if (!chapter || typeof chapter.content_html !== 'string') {
      this.renderCurrentChapter(() => this.playAudio());
      return;
    }
  ...
  loadAvatars() {
    this.avatarsRequestSub?.unsubscribe();
    this.avatarsRequestSub = this.api.get(`ai/avatars/?inventory_id=${this.inventoryId}`).pipe(takeUntil(this.destroy$)).subscribe({
