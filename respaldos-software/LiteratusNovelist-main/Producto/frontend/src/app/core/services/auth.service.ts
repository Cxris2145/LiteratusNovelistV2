...
export class AuthService {
  ...
  login(identifier: string, password: string): Observable<AuthResponse> {
    ...
    return this.api.post<AuthResponse>('users/login/', payload);
  }
  ...
  requestPasswordReset(email: string): Observable<any> {
    return this.api.post(`users/password-reset/`, { email: this.normalizeEmail(email) });
  }

  validatePasswordReset(uid: string, token: string): Observable<any> {
    return this.api.post(`users/password-reset-validate/`, { uid, token });
  }

  confirmPasswordReset(uid: string, token: string, new_password: string, confirm_password?: string): Observable<any> {
    return this.api.post(`users/password-reset-confirm/`, {
      uid,
      token,
      new_password,
      confirm_password
