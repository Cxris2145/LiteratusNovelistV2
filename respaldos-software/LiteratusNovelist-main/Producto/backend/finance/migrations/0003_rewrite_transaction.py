...
def drop_old_finance_tables(apps, schema_editor):
    table_names = [
        'finance_transaction',
        'finance_orderitem',
        'finance_order',
    ...
    with schema_editor.connection.cursor() as cursor:
        for table_name in table_names:
            cursor.execute(f'DROP TABLE IF EXISTS {table_name}{suffix};')
...
class Migration(migrations.Migration):
    ...
    operations = [
        # 1. Borrar TODAS las tablas viejas (si existen) segun el motor local
        migrations.RunPython(
            drop_old_finance_tables,
