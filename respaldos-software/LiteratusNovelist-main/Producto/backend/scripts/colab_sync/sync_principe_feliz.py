...
def process_chapter_audio(chapter_order, audio_file_path, voice_name):
    chapter = Chapter.objects.filter(book__title__icontains='Príncipe Feliz', order=chapter_order).first()
    if not chapter:
...
if __name__ == "__main__":
    # Procesar capítulos 1 a 5 (orders 0 a 4)
    for i in range(1, 6):
        process_chapter_audio(
            chapter_order=i - 1, # Capitulo_01.mp3 -> order 0
            audio_file_path=f"audio_narrations/Principe_Feliz/Capitulo_0{i}.mp3",
            voice_name="Caro (Voz Grabada)"
        )
