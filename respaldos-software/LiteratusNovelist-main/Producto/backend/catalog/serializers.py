...
class EditionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Edition
        fields = ['id', 'language', 'isbn', 'format', 'price', 'published_date', 'publisher', 'display_name', 'book']

    def get_display_name(self, obj):
        return f"{obj.get_format_display()} - {obj.language.upper()}"
...
class BookDetailFullSerializer(BookDetailSerializer):
    ...
    def get_avatars(self, obj):
        ...
        prefetched_editions = getattr(obj, '_prefetched_objects_cache', {}).get('editions')
        if prefetched_editions is not None:
            for edition in prefetched_editions:
                for avatar in edition.avatars.all():
    ...
    def get_total_words(self, obj):
        """Estima palabras sin cargar todos los capítulos completos en memoria."""
        if hasattr(obj, '_cached_total_words'):
            return obj._cached_total_words
        ...
        if not samples:
            obj._cached_total_words = len((obj.synopsis or '').split()) * 5
            return obj._cached_total_words

        sample_words = sum(len(content.split()) for content in samples if content)
        if chapter_count > len(samples) and len(samples) > 0:
            total_words = round((sample_words / len(samples)) * chapter_count)
        else:
            total_words = sample_words

        obj._cached_total_words = total_words
        return total_words
    ...
    def get_is_owned(self, obj):
        return self._get_user_inventory(obj) is not None

    def get_inventory_id(self, obj):
        inv = self._get_user_inventory(obj)
        if inv:
            return str(inv.id)
        return None
    ...
    def get_has_premium_narration(self, obj):
        inv = self._get_user_inventory(obj)
        if inv:
            return inv.has_premium_narration
        return False
    ...
    def _get_user_inventory(self, obj):
        if hasattr(obj, '_cached_request_inventory'):
            return obj._cached_request_inventory

        request = self.context.get('request')
        if not request or not request.user.is_authenticated:
            obj._cached_request_inventory = None
            return None

        from library.models import UserInventory
        obj._cached_request_inventory = (
            UserInventory.objects
            .filter(user=request.user, edition__book=obj)
            .only('id', 'has_premium_narration')
            .first()
        )
        return obj._cached_request_inventory
