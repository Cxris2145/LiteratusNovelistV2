...
class CatalogAPITests(APITestCase):
    def setUp(self):
        ...
        self.draft_book = Book.objects.create(
            ...
            synopsis='Contenido no publicado.',
    ...
    def test_books_list_excludes_unpublished_books(self):
        response = self.client.get('/api/v1/catalog/books/')

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        titles = {book['title'] for book in response.data['results']}
        self.assertIn('Cien Anos de Soledad', titles)
        self.assertNotIn('Borrador Privado', titles)

    def test_unpublished_book_is_not_retrievable_by_slug(self):
        response = self.client.get(f'/api/v1/catalog/books/{self.draft_book.slug}/')
    ...
    def test_compact_catalog_returns_only_card_fields(self):
        response = self.client.get('/api/v1/catalog/books/?compact=true')

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            set(response.data['results'][0]),
            {'id', 'title', 'slug', 'synopsis', 'is_featured', 'cover_image'},
        )
